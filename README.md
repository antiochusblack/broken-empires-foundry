...
An unofficial, lightweight Foundry VTT v14 game system. Version 0.2.1 includes the character creation and character sheet fields, editable skills, Expertise and Savvy, custom skills/resources, fatigue and wounds, and Item-based talents and weapons. Character creation bonuses, combat and rolls are entered manually for now. This update fixes section navigation, preserves scroll position during edits, and aligns list headings and talent rows.
...
Commit these files to `main` and create a GitHub release tagged `v0.2.1` with the asset `broken-empires-foundry-v0.2.1.zip`. The archive contains a top-level `broken-empires-foundry/` folder with `system.json` immediately inside. Restart your world after updating the system.
